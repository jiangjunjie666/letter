<template>
  <div class="communityCard">
    ...
    <div class="text">
      ...
      <!-- 图片墙 -->
      <div class="imgList">
        <img :key="i" v-for="i in communityList.images" :src="i" />
      </div>
      <!-- 等人赞过 -->
    ...
    <div class="buttom">
      <ul>
        ...
        <li>
          <el-icon><Star /></el-icon>{{ communityList.collect }}
        </li>
      </ul>
    </div>
  </div>
  ...
  <div class="review" v-if="reviewOpen">
    ...
    <div class="reviewText">
      ...
      <div class="text">
        <el-input v-model="reviewText" class="input" maxlength="500" placeholder="Please input" show-word-limit type="textarea" />
      </div>
    </div>
    ...
    <div class="allReview">
      <div class="sort">
        <h1 :class="{ active: sortReviewActive == 'hot' }" @click="sortReviewActive = 'hot'">最热</h1>
        <h1>|</h1>
        <h1 :class="{ active: sortReviewActive == 'new' }" @click="sortReviewActive = 'new'">最新</h1>
      </div>
      <!-- 具体的评论 -->
      ...
      <!-- 底部展开更多评论 -->
      <el-button v-if="commentList.length < totalReview" class="more" @click="moreReview">查看全部{{ totalReview - commentList.length }}条评论</el-button>
    </div>
  </div>
</template>
...
import { reqLikeComment, reqPostComment, reqGetComment, reqGetLikeList } from '@/api/module/community.js'
...
const sendPostComment = async () => {
  let res = await reqPostComment(reviewText.value, props.communityList.tid, userStore.userInfo.userId)
  if (res.code !== 1) {
    return ElMessage({
      type: 'error',
      message: '发布评论失败'
...
const getComment = async () => {
  let res = await reqGetComment(props.communityList.tid, pageReview.value, pageSizeReview.value)
  if (res.code !== 1) {
    return ElMessage({
      type: 'error',
      message: '获取评论失败'
...
const getLikeList = async () => {
  let res = await reqGetLikeList(props.communityList.tid, pageLike.value, pageSizeLike.value)
  if (res.code != 1) {
    return ElMessage({
      type: 'error',
      message: '获取点赞列表失败'
